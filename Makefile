TEST?=$(shell go list -f '{{.ImportPath}}/...' ./... | grep -v /vendor/ | sed "s|$(shell go list -f '{{.ImportPath}}' .)|.|g" | sed "s/\.\/\.\.\./\.\//g")
...
vet:
	...
	@go tool vet $(VETARGS) $(VET) ; if [ $$? -eq 1 ]; then \
		echo ""; \
...
lint:
	...
	@sh -c "'$(CURDIR)/scripts/golint.sh' ${VET}"; if [ $$? -eq 1 ]; then \
		echo ""; \
